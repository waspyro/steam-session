...
export default class SteamSession {
  ...
  static async Auto(login, password, actor, sessiontype = 'web', logurl, logcookies, storageDriver) {
    const session = new SteamSession()
    if(!['web', 'client', 'mobile'].includes(sessiontype)) throw new Error('unknown session type')
    if(logurl) session.onrequest = url => console.log('>', url.toString())
    if(logcookies) session.cookies.on('set', cookie => console.log('+cookie', cookie.domain, cookie.name))
    if(logcookies) session.cookies.on('del', cookie => console.log('-cookie', cookie.domain, cookie.name))
    if(!storageDriver) await session.useSessionSaveLocation(os.homedir(), '.cache', 'steam-session', login, sessiontype)
    else await session.useStorageDriver(storageDriver)
    if(!session.env) switch(sessiontype) {
      case 'web': session.setEnv(webBrowser()); break
      case 'mobile': session.setEnv(mobileIOS()); break
      case 'client': session.setEnv(clientWindows()); break
    }
    if(session.isRefreshTokenExpired()) await session.getJWTViaCredentials(login, password, actor)
    if(session.cookies.isExpired()) await session.refreshCookies()
    session.startRefreshInterval()
    return session
  }
