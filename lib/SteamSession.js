...
import {parseSetCookie} from "cookie-store";
...
export default class SteamSession {
  ...
  request(url, opts = {}) {
    opts = Object.assign({}, opts, this.requestOpts) //todo
    ...
    return this.#processRequest(url, opts)
  }

  async #processRequest(url, opts) {
    const response = await fetch(url, opts)
    response.cookies = response.headers.has('set-cookie') //todo: https://github.com/nodejs/undici/issues/1437
      ? parseSetCookie(response.headers.get('set-cookie'), url) : []
    if(response.cookies.length) {
      if(opts.saveResponseCookies === undefined || opts.saveResponseCookies === true)
        this.cookies.set(response.cookies, url)
      if(typeof opts.saveResponseCookies === 'function')
        this.cookies.set(response.cookies.filter(opts.saveResponseCookies), url)
    }
    ...
    if(opts.followRedirects !== undefined && (response.status === 301 || response.status === 302)) {
      ...
      opts.headers.Cookie = this.cookies.get(newLocation).toString()
      if(opts.followRedirects--) return this.#processRequest(newLocation, opts)
  ...
  async refreshCookies(refreshToken, domains) { //todo: domains "all", "any"
    ...
    const urlsWithoutAccessCookie = []
    const results = await Promise.all(transfer_info.map(async el => {
      ...
      const access = el.cookies.find(el => el.name === 'steamLoginSecure')
      if(!access) urlsWithoutAccessCookie.push(el.url)
      else accessCookie = access
      return el
    }))

    if(!accessCookie) throw new Error('unable obtain access cookie')
    accessCookie.domain = undefined
    urlsWithoutAccessCookie.forEach(url => this.cookies.set(accessCookie, url))
  ...
  startRefreshInterval(subTime = 1000 * 60 * 60 * 12) { //half a day
    ...
    if(mcTillUpdateRequired <= 0)
      this.refreshCookies().then(() =>
        this.#refreshInterval = setTimeout(() => this.startRefreshInterval(subTime), subTime))
    else this.#refreshInterval = setTimeout(() => this.startRefreshInterval(subTime), mcTillUpdateRequired)
  }

  stopRefreshInterval = () => clearTimeout(this.#refreshInterval)

  async init(login, password, actor, type = 'web', storage, debug = {}, requestOpts = {}) {
    const debugPreset = debug.preset ? debug.preset + ':' + type : type

    if(debug.requests) {
      this.onrequest = (url, opts) => console.log(debugPreset, '> ', url.toString())
      if(debug.requests === 'full') this.onrequest =
        (url, opts) => console.log(debugPreset, '> ', url.toString(), '\n', opts)
      else if(typeof debug.requests === 'function')
        this.onrequest = debug.requests
    }

    this.requestOpts = requestOpts

    if(debug.cookies) {
      this.cookies.on('set', cookie => console.log(debugPreset, '+cookie', cookie.domain, cookie.name))
      this.cookies.on('del', cookie => console.log(debugPreset, '-cookie', cookie.domain, cookie.name))
    }
    ...
    if(this.isRefreshTokenExpired()) await this.getJWTViaCredentials(login, password, actor)
      .then(() => this.refreshCookies())
    else if(this.cookies.isExpired()) await this.refreshCookies()
