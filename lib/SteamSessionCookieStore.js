...
export default class SteamSessionCookieStore extends CookieStore {
  ...
  getAccessCookies() {
    const cookies = []
    this.forEach(cookie => cookie.name === 'steamLoginSecure' && cookies.push(cookie))
    return cookies
  }

  getAccessCookiesTokens() {
    return this.getAccessCookies().map(el => el.value.split('%7C%7C')[1])
  }

  getSteamID() {
    const cookies = this.getAccessCookies()
    if(!cookies.length) return null
    return cookies[0].value.split('%7C%7C')[0]
  }
